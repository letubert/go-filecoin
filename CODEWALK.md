...
A miner packs pieces from many clients into a sector, which is then sealed with a proof (aka commitment). 
...
At present, retrieval is not charged for, and always causes unsealing of a sector.

Note that due to use of the IPFS block service for transmitting pieces, a client can issue a blockservice `get` which will find pieces in the block service of the local or any other node, 
and fetch them without invoking retrieval protocol. 
With time, the block service will evict stale blocks, so this is not reliable. 
The `client cat` command simply fetches blocks from the block service; the `retrieval-client retrieve-piece` invokes retrieval protocol proper.
